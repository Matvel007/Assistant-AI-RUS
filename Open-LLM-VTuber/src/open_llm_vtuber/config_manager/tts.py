...
class SileroTTSConfig(I18nMixin):
    """Configuration for Silero TTS."""

    language: str = Field("ru", alias="language")
    speaker: str = Field("v5_ru", alias="speaker")
    device: str = Field("cpu", alias="device")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "language": Description(
            en="Language code (ru, en, de, es, fr, uk, uz, cyrillic)",
            zh="语言代码（ru, en, de, es, fr, uk, uz, cyrillic）",
        ),
        "speaker": Description(
            en="Speaker name (e.g., v5_ru, aidar, baya, kseniya, irina, natasha, ruslan)",
            zh="说话人名称（例如 v5_ru, aidar, baya, kseniya, irina, natasha, ruslan）",
        ),
        "device": Description(
            en="Device to use (cpu or cuda)", zh="使用的设备（cpu 或 cuda）"
        ),
    }
...
class TTSConfig(I18nMixin):
    ...
    silero_tts: Optional[SileroTTSConfig] = Field(None, alias="silero_tts")  # <-- добавлено
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        ...
        "silero_tts": Description(  # <-- добавлено
            en="Configuration for Silero TTS", zh="Silero TTS 配置"
        ),
    }
    ...
    def check_tts_config(cls, values: "TTSConfig", info: ValidationInfo):
        ...
        elif tts_model == "silero_tts" and values.silero_tts is not None:  # <-- добавлено
