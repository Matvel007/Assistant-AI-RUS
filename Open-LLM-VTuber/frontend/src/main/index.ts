...
app.whenReady().then(() => {
  ...
  // Регистрируем глобальную горячую клавишу для переключения микрофона
  const micToggleShortcut = 'Alt+X'; 
  const ret = globalShortcut.register(micToggleShortcut, () => {
    console.log(`Глобальная клавиша ${micToggleShortcut} нажата`);
  ...
  if (!ret) {
    console.log(`Не удалось зарегистрировать горячую клавишу ${micToggleShortcut}`);
  } else {
    console.log(`Горячая клавиша ${micToggleShortcut} зарегистрирована`);
  }
  // ==============================================================
